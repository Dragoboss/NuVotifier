...
public class FileVoteCache extends MemoryVoteCache {
    ...
    private void load() throws IOException {
        ...
        if (!object.has("players") || !object.has("servers") || !object.has("version") || object.getAsJsonArray().size()!= 3) {
        ...
        for (Map.Entry<String, JsonElement> player : players.entrySet()) {
            playerVoteCache.put(player.getKey(), readVotes(players.getAsJsonArray(player.getKey())));
        }

        for (Map.Entry<String, JsonElement> server : servers.entrySet()) {
            voteCache.put(server.getKey(), readVotes(servers.getAsJsonArray(server.getKey())));
        }
